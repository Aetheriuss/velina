...
public class UsersService : ServiceBase, IService
{
    ...
    public async Task<UserId> CreateUser(string username, string password, Gender gender, long? overrideUserId = null, string? discordId = null)
    {
        ...
        var result = await InTransaction(async _ =>
        {
            ...
            // Registration is open (Discord-only); no application approval is required anymore.
    ...
    // The invite system was removed; the user_invite table no longer exists. These reads
    // are retained as no-op stubs because anti-abuse logic (purchase/currency/trade checks)
    // still calls them — with invites gone, every user simply has none.
    public Task<IEnumerable<UserInviteEntry>> GetInvitesByUser(long userId)
    {
        return Task.FromResult(Enumerable.Empty<UserInviteEntry>());
    }

    public Task<UserInviteEntry?> GetUserInvite(long userId)
    {
        return Task.FromResult<UserInviteEntry?>(null);
    }
